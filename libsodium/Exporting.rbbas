...
Private Module Exporting
	...
	#tag Method, Flags = &h1
		Protected Function Export(KeyData As MemoryBlock, Type As libsodium.Exporting.ExportableType, EncryptWith As libsodium.PKI.EncryptionKey, StreamType As libsodium.KeyStream.StreamType, Optional MetaData As Dictionary) As MemoryBlock
		  ' This method takes a raw binary crypto key and encodes it in a plain text
		  ' format that is suitable to be stored or transferred. The KeyData will be 
		  ' encrypted with a random nonce and the EncryptWith key. This key will be 
		  ' needed to import the key again. The nonce will be encoded in the exported 
		  ' data along with any additional data in the optional MetaData dictionary. 
		  ' The keys and values in the dictionary must all be strings.
		  
		  Dim data As New MemoryBlock(0)
		  Dim output As New BinaryStream(data)
		  output.Write(GetPrefix(Type) + EndOfLine.Windows)
		  Dim fk As New libsodium.PKI.ForeignKey(EncryptWith)
		  Dim stream As New libsodium.KeyStream(fk, StreamType)
		  Dim nonce As MemoryBlock = stream.RandomNonce
		  KeyData = stream.Process(KeyData, nonce)
		  output.Write("#Nonce=")
		  output.Write(libsodium.EncodeHex(nonce))
		  output.Write(EndOfLine.Windows)
		  Select Case StreamType
		  Case libsodium.KeyStream.StreamType.ChaCha20
		    output.Write("#Alg=ChaCha20" + EndOfLine.Windows)
		  Case libsodium.KeyStream.StreamType.Salsa20
		    output.Write("#Alg=Salsa20" + EndOfLine.Windows)
		  Case libsodium.KeyStream.StreamType.XChaCha20
		    output.Write("#Alg=XChaCha20" + EndOfLine.Windows)
		  Case libsodium.KeyStream.StreamType.XSalsa20
		    output.Write("#Alg=XSalsa20" + EndOfLine.Windows)
		  End Select
		  ...
		  For i As Integer = 0 To UBound(lines)
		    ...
		    If Left(s, 1) = "#" Then ' meta line
		      ...
		      Case Left(s, 5) = "#Alg="
		        ...
		        Case "ChaCha20"
		          MetaData.Value("Alg") = KeyStream.StreamType.ChaCha20
		        Case "Salsa20"
		          MetaData.Value("Alg") = KeyStream.StreamType.Salsa20
		        Case "XChaCha20"
		          MetaData.Value("Alg") = KeyStream.StreamType.XChaCha20
		        Case "XSalsa20"
		          MetaData.Value("Alg") = KeyStream.StreamType.XSalsa20
		        End Select
	...
	#tag Method, Flags = &h1
		Protected Function Import(EncodedKey As MemoryBlock, EncryptedWith As libsodium.PKI.EncryptionKey) As MemoryBlock
		  ' This method takes a key encoded in the plain text format generated by the Export method
		  ' and returns the raw binary data of the key. If a Password is specified then the key will
		  ' be decrypted with a SecretKey derived from the password.
		  ...
		  Dim metadata As Dictionary = GetMetaData(EncodedKey)
		  EncodedKey = GetKeyData(EncodedKey, GetType(EncodedKey))
		  If metadata.HasKey("Nonce") And metadata.HasKey("Alg") Then
		    If EncryptedWith = Nil Then Raise New SodiumException(ERR_IMPORT_ENCRYPTED)
		    Dim n As MemoryBlock = metadata.Value("Nonce")
		    Dim a As libsodium.KeyStream.StreamType = metadata.Lookup("Alg", libsodium.KeyStream.StreamType.XSalsa20)
		    Dim fk As New libsodium.PKI.ForeignKey(EncryptedWith)
		    Dim stream As New libsodium.KeyStream(fk, a)
		    EncodedKey = stream.Process(EncodedKey, n)
		    If EncodedKey = Nil Then Raise New SodiumException(ERR_IMPORT_PASSWORD)
		  End If
